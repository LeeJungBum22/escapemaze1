...
public class TMP_FontChangerEditor : Editor
{
    public override void OnInspectorGUI()
    {
        ...
        if (GUILayout.Button("Change Font!"))
        {
            ...
            // 🌟 최신 유니티 문법으로 수정됨
            foreach(TextMeshPro textMeshPro3D in GameObject.FindObjectsByType<TextMeshPro>(FindObjectsInactive.Include, FindObjectsSortMode.None)) 
            { 
                textMeshPro3D.font = fontAsset;
            }
            foreach(TextMeshProUGUI textMeshProUi in GameObject.FindObjectsByType<TextMeshProUGUI>(FindObjectsInactive.Include, FindObjectsSortMode.None)) 
            { 
